...
public class ShotCutTest{
    ...
    public void testCut1(){
        ...
        IMediaResourceItem item = TestUtils.createVideoItem(dir + File.separator + videoName);
        ArrayList<IMediaResourceItem> list = new ArrayList<>(Arrays.asList(item));
        testCutShot(list);
    }

    private List<MediaPartItem> cutItems(List<IMediaResourceItem> items, CyclicBarrier barrier) {
