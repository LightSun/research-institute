...
public class ElfType32 {
	...
	public static class elf32_hdr{
		public byte[] e_ident = new byte[16];  //魔数
		...
		//全名：Program Header offset，程序头-内容在整个文件的偏移值，我们可以用这个偏移值来定位程序头的开始位置，用于解析程序头信息
		public byte[] e_phoff = new byte[4];
		//Section Header offset, 段头内容在这个文件的偏移值，我们可以用这个偏移值来定位段头的开始位置，用于解析段头信息
