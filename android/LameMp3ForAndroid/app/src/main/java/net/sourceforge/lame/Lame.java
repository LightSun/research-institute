...
public class Lame {
    ...
    private static final String LAME_LIB = "lamemp3";
     ...
     * @return -1 if error occurred, number of bytes decoded otherwise
     */
    public static int decodeFrame2(InputStream input,
                                  float[] pcmLeft, float[] pcmRight) throws IOException {
        int len = 0;
        int samplesRead = 0;
        byte[] buf = new byte[MP3_BUFFER_SIZE];

        // check for buffered data
        samplesRead = nativeDecodeFrame2(buf, len, pcmLeft, pcmRight);
        if (samplesRead != 0) {
            return samplesRead;
        }
        while (true) {
    ...
    private static native int nativeDecodeFrame2(byte[] inputBuffer, int bufferSize,
                                                float[] pcmLeft, float[] pcmRight);
