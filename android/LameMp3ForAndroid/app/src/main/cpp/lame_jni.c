...
JNIEXPORT jint JNICALL Java_net_sourceforge_lame_Lame_nativeDecodeFrame2
        (JNIEnv *env, jclass class, jbyteArray mp3Buffer, jint bufferSize,
         jfloatArray leftChannel, jfloatArray rightChannel)
{
    int samples_read;
    float *left_buf, *right_buf;
    unsigned char *mp3_buf;

    left_buf = (*env)->GetFloatArrayElements(env, leftChannel, NULL);
    right_buf = (*env)->GetFloatArrayElements(env, rightChannel, NULL);
    mp3_buf = (*env)->GetByteArrayElements(env, mp3Buffer, NULL);

    samples_read = hip_decode1_unclipped2(hip_context, mp3_buf, bufferSize, left_buf, right_buf, mp3data);

    (*env)->ReleaseByteArrayElements(env, mp3Buffer, mp3_buf, 0);

    if (samples_read < 0) {
        // some sort of error occurred, don't propagate changes to buffers
        (*env)->ReleaseFloatArrayElements(env, leftChannel, left_buf, JNI_ABORT);
        (*env)->ReleaseFloatArrayElements(env, rightChannel, right_buf, JNI_ABORT);
        return samples_read;
    }

    (*env)->ReleaseFloatArrayElements(env, leftChannel, left_buf, 0);
    (*env)->ReleaseFloatArrayElements(env, rightChannel, right_buf, 0);
