...
abstract class BaseAndroidClassLoader extends ClassLoader implements GeneratedClassLoader {
    ...
    private final DxContext mDxContext = new DxContext();
    /**
    ...
    public Class<?> defineClass(String name, byte[] data) {
        try {
            ...
            classFile.getMagic();
            dexFile.add(CfTranslator.translate(mDxContext, classFile, null, new CfOptions(), dexOptions, dexFile));
            Dex dex = new Dex(dexFile.toDex(null, false));
            Dex oldDex = getLastDex();
            if (oldDex != null) {
                dex = new DexMerger(new Dex[]{dex, oldDex}, CollisionPolicy.KEEP_FIRST, mDxContext).merge();
            }
