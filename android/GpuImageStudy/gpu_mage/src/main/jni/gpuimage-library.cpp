...
Java_jp_co_cyberagent_android_gpuimage_GPUImageNativeLibrary_YUVtoRBGA(JNIEnv *env, jclass type,
    ...
    jint *rgbData = (*env)->GetIntArrayElements(env, out_, NULL);
    ...
    (*env)->ReleaseIntArrayElements(env, out_, rgbData, 0);
}

JNIEXPORT void JNICALL
Java_jp_co_cyberagent_android_gpuimage_GPUImageNativeLibrary_YUVtoARBG(JNIEnv *env, jclass type,
                                                                       jbyteArray yuv_, jint width,
                                                                       jint height,
                                                                       jintArray out_) {
    jbyte *yuv = (*env)->GetByteArrayElements(env, yuv_, NULL);
