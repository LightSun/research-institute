--[[--
 __index is not followed when looking for metamethods:
--]]
...
setmetatable(DerivedClass, {
    __index = BaseClass, -- this is what makes the inheritance work 继承实现的关键点
    __call = function (cls, ...)
        local self = setmetatable({}, cls)
        self:_init(...)
        return self
...
function DerivedClass:_init(init1, init2)
    BaseClass._init(self, init1) -- call the base class constructor. 调用父构造
    self.value2 = init2
end
