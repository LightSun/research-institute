...
local function save_sms(fp, ctnt)
 ...
 while(lineb and linee) do
  ...
  elseif(string.find(line, archive_box)) then
   ...
   local hb, he = string.find(line, head)
   local tb, te = string.find(line, tail)

   local first_number = ""
   -- service center address
   local sb, se = string.find(line, phone_nb, tb)
   --print("" .. sb .. ", " .. se)
   if(sb and se) then
   ...
   if(type ~= BoxType.OUT and first_number ~= "empty number") then
    local tmb, tme = string.find(line, time, se)
    --print('' .. tmb .. ", " .. tme)
    if(tmb and tme) then
...
local function save_nfx_to(from, too)
    ...
    local v = string.format('%c%c', 255, 254)
	-- unicode .txt 'FF FE'
	fp:write(v)
