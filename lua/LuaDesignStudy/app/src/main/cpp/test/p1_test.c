...
static int test_main01(struct lua_State* L) {
    ...
    LOGD("test_main01 luaL_tointeger value = %d\n", i);
    return 0;
}
...
static int test_main02(struct lua_State* L) {
    ...
    LOGD("test_main02 luaL_tointeger value = %d\n", i);
    return 1;
}
...
void p1_test_result02() { // nwant = 0; and nresult = 1;
    ...
    LOGD("p1_test_result02 before push args test_main02 stacksize = %d\n", luaL_stacksize(L));

    luaL_pushcfunction(L, &test_main02);
    luaL_pushinteger(L, 1);
    
    LOGD("p1_test_result02 before call test_main02 stacksize = %d\n", luaL_stacksize(L));

    luaL_pcall(L, 1, 0);
    
    LOGD("p1_test_result02 after call test_main02 stacksize = %d\n", luaL_stacksize(L));
...
void p1_test_result03() { // nwant = 0; and nresult > 1;
    struct lua_State* L = luaL_newstate();
    LOGD("p1_test_result03 before push arg test_main03 stacksize = %d\n", luaL_stacksize(L));
    luaL_pushcfunction(L, &test_main03);
    luaL_pushnumber(L, 1.0f);
    luaL_pushboolean(L, true);
    LOGD("p1_test_result03 before call test_main03 stacksize = %d\n", luaL_stacksize(L));
    luaL_pcall(L, 2, 0);
    LOGD("p1_test_result03 after call test_main03 stacksize = %d\n", luaL_stacksize(L));
    luaL_close(L);
}
...
static int test_main04(struct lua_State* L) {
    ...
    LOGD("test_main04 arg1:%d arg2:%d\n", arg1, arg2);
...
void p1_test_result04() { // nwant = 1; and nresult = 0;
    struct lua_State* L = luaL_newstate();
    LOGD("p1_test_result04 before push arg test_main04 stacksize = %d\n", luaL_stacksize(L));
    luaL_pushcfunction(L, &test_main04);
    luaL_pushinteger(L, 1);
    luaL_pushinteger(L, 2);
    LOGD("p1_test_result04 before call test_main04 stacksize = %d\n", luaL_stacksize(L));
    luaL_pcall(L, 2, 1);
    LOGD("p1_test_result04 after call test_main04 stacksize = %d\n", luaL_stacksize(L));

    int isnil = luaL_isnil(L, -1);
    luaL_pop(L);
    LOGD("p1_test_result04 isnil:%d\n", isnil); 
    LOGD("p1_test_result04 stacksize = %d\n", luaL_stacksize(L));
...
void p1_test_result05() { // nwant = 1; and nresult = 1;
    ...
    LOGD("p1_test_result05 top_value:%d\n", b?1:0);
    luaL_pop(L);

    LOGD("p1_test_result05 before close stack_size:%d\n", luaL_stacksize(L));
    luaL_close(L);
}
...
void p1_test_result06() { // nwant = 1; and nresult > 1;
    ...
    LOGD("p1_test_result06 after call stacksize:%d\n", luaL_stacksize(L));
    lua_Integer v = luaL_tointeger(L, -1);
    LOGD("p1_test_result06 top value:%d\n", v);
    luaL_pop(L);
    LOGD("p1_test_result06 final stacksize:%d\n", luaL_stacksize(L));
...
void p1_test_result07() { // nwant > 1; and nresult = 0;
    ...
    LOGD("p1_test_result07 after call stack_size:%d\n", luaL_stacksize(L));
    int i;
    for (i = 0; i < test_result07_nwant; i++) {
        int isnil = luaL_isnil(L, -1);
        luaL_pop(L);
        LOGD("p1_test_result07 stack_idx:%d isnil:%d\n", test_result07_nwant - i, isnil);
    } 
    LOGD("p1_test_result07 final stack_size:%d\n", luaL_stacksize(L));
...
void p1_test_result08() { // nwant > 1; and nresult > 0;
    ...
    LOGD("p1_test_result08 after call stack_size:%d\n", luaL_stacksize(L));
    int i;
    for (i = 0; i < test_result08_nwant; i++) {
        if (i != 0) {
            lua_Integer integer = luaL_tointeger(L, -1);
            luaL_pop(L);
            LOGD("p1_test_result08 stack_idx:%d integer:%d\n", test_result08_nwant - i, integer);
        }
        else {
            int isnil = luaL_isnil(L, -1);
            luaL_pop(L);
            LOGD("p1_test_result08 stack_idx:%d isnil:%d\n", test_result08_nwant - i, isnil);
        }
    } 
    LOGD("p1_test_result08 final stack_size:%d\n", luaL_stacksize(L));
...
void p1_test_result09() { // nwant = -1; and nresult = 0;
    ...
    LOGD("p1_test_result09 before call  stack_size:%d\n", luaL_stacksize(L));
    luaL_pcall(L, 0, -1);
    LOGD("p1_test_result09 after call stack_size:%d\n", luaL_stacksize(L));
    luaL_close(L);
}
...
void p1_test_result10() { // nwant = -1; and nresult > 0;
    ...
    LOGD("p1_test_result10 after call stack_size:%d\n", stack_size);
    ...
    LOGD("p1_test_result10 final stack_size:%d\n", luaL_stacksize(L));
...
static int test_nestcal01(struct lua_State* L) {
    ...
    //LOGD("test_nestcal01 enter_calls %d\n", current_calls);
    ...
    //LOGD("test_nestcal01 current_calls:%d stack_size:%d\n", temp, luaL_stacksize(L));
...
void p1_test_nestcall01() { // call count < LUA_MAXCALLS
    ...
    LOGD("p1_test_nestcall01 result = %d stack_size:%d\n", luaL_tointeger(L, -1), luaL_stacksize(L));
    luaL_pop(L);
